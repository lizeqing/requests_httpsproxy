    ...
    def _new_conn(self):
        ...
        try:
            ...
            proxy_sock = create_socket_connection(
                (proxy_host, proxy_port), timeout=self.timeout, **extra_kw)
            ...
            while True:
                ...
                if not s:
                    if code == 0:
                        raise SocketError(
                            "Tunnel connection failed: %r" % data)
                    break
                data += s
                if code == 0 and b'\r\n' in data:
...
class HTTPSProxyProxyManager(PoolManager):
    ...
    def __init__(self,
        ...
        super(HTTPSProxyProxyManager, self).__init__(num_pools, headers,
                                                     **connection_pool_kw)
    ...
    def _new_pool(self, scheme, host, port, request_context=None):
        ...
        # pass _ssl_options here
        request_context['_ssl_options'] = self._ssl_options
